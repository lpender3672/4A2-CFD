...
def check_run_converged(content):

    for line in content:
        if 'NaN detected' in line: # diverged
            return 2
    for line in content:
        if 'converged outside bounds' in line:
            return 1
    for line in content:
        if 'converged within bounds' in line:
            return 0

    # exit code 3 means did not converge, nor diverge
    # check inputs
    return 3
